...
class StockPickingWave(models.Model):
    ...
    scheduled_date_start = fields.Datetime(string="Scheduled Date Start")
    scheduled_date_end = fields.Datetime(string="Scheduled Date End")
    creation_date_start = fields.Datetime(string="Creation Date Start")
    creation_date_end = fields.Datetime(string="Creation Date End")

    @api.constrains("scheduled_date_start", "scheduled_date_end")
    def _check_scheduled_date(self):
        strWarning = _("Scheduled Date Start must be greater than Scheduled Date End")
        if self.scheduled_date_start and self.scheduled_date_end:
            if self.scheduled_date_start > self.scheduled_date_end:
                raise UserError(strWarning)

    @api.constrains("creation_date_start", "creation_date_end")
    def _check_creation_date(self):
        strWarning = _("Creation Date Start must be greater than Creation Date End")
    ...
    def _prepare_picking_search(self):
        ...
        if scheduled_date_start:
            criteria.append(("min_date", ">=", scheduled_date_start))
        if scheduled_date_end:
            criteria.append(("min_date", "<=", scheduled_date_end))
        if creation_date_start:
            criteria.append(("date", ">=", creation_date_start))
        if creation_date_end:
            criteria.append(("date", "<=", creation_date_end))
        if delivery_address_ids:
            criteria.append(("delivery_address_id", "in", delivery_address_ids.ids))
        if picking_type_ids:
            criteria.append(("picking_type_id", "in", picking_type_ids.ids))
        return criteria
    ...
    def button_search_pickings(self):
        ...
        criteria.append(("wave_id", "=", False))
        criteria.append(("state", "not in", ("cancel", "done")))
