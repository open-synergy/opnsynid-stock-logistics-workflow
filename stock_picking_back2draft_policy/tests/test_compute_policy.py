...
class TestComputePickingBack2DraftPolicy(TransactionCase):
    def setUp(self, *args, **kwargs):
        super(TestComputePickingBack2DraftPolicy, self).setUp(*args, **kwargs)
        # Objects
        ...
        # Add Group Button Back To Draft
        self.grp_back2draft = self.obj_res_groups.create({
            'name': 'Group Button Back To Draft'
        })
    ...
    def test_compute_case_admin(self):
        ...
        self.assertEqual(True, picking.back2draft_ok)
    ...
    def test_compute_case_1(self):
        ...
        self.picking_type_out.back2draft_group_ids = [(
            6, 0, [
                self.grp_back2draft.id
            ]
        )]

        # Result
        #   1. User Test cannot Back To Draft

        self.assertEqual(
            False,
            picking.sudo(
                self.user.id).back2draft_ok
        )

        # Condition :
        #   1. User Test have group
        # Add Group Back To Draft
        self.user.groups_id = [(
            4,
            self.grp_back2draft.id
        )]
