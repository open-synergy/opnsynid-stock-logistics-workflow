...
class TestComputePickingPolicy(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.grp_return = self.obj_res_groups.create({
            'name': 'Group Button Return'
        })
        # Add Group Button Done
        self.grp_cancel = self.obj_res_groups.create({
            'name': 'Group Button Cancel'
        })
    ...
    def test_compute_case_1(self):
        ...
        self.picking_type_out.confirm_group_ids = [(
            6, 0, [
                self.grp_confirm.id
            ]
        )]

        self.picking_type_out.force_group_ids = [(
            6, 0, [
                self.grp_force.id
            ]
        )]
        ...
        self.assertEqual(
            False,
            picking.sudo(
                self.user_1.id).confirm_ok
        )
        self.assertEqual(
            False,
            picking.sudo(
                self.user_1.id).force_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_1.id).transfer_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_1.id).return_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
    ...
    def test_compute_case_2(self):
        ...
        self.picking_type_out.transfer_group_ids = [(
            6, 0, [
                self.grp_transfer.id
            ]
        )]

        self.picking_type_out.return_group_ids = [(
            6, 0, [
                self.grp_return.id
            ]
        )]
        ...
        self.assertEqual(
            True,
            picking.sudo(
                self.user_2.id).confirm_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_2.id).force_ok
        )
        self.assertEqual(
            False,
            picking.sudo(
                self.user_2.id).transfer_ok
        )
        self.assertEqual(
            False,
            picking.sudo(
                self.user_2.id).return_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
    ...
    def test_compute_case_3(self):
        ...
        self.picking_type_out.cancel_group_ids = [(
            6, 0, [
                self.grp_cancel.id
            ]
        )]

        self.picking_type_out.unreserve_group_ids = [(
            6, 0, [
                self.grp_unreserve.id
            ]
        )]
        # Result
        ...
        self.assertEqual(
            True,
            picking.sudo(
                self.user_3.id).confirm_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_3.id).force_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_3.id).transfer_ok
        )
        self.assertEqual(
            True,
            picking.sudo(
                self.user_3.id).return_ok
        )
        self.assertEqual(
            False,
            picking.sudo(
